...
function renderVendorLevel2(request = {}) {
  ...
  return '<div class="model-index-page vendor-preview-level2">' +
    '<button class="model-index-back" type="button" aria-label="返回上一级" title="返回上一级" onclick="goBackModelToolPanel(\'' + escapeHtml(preview.vendor_key) + '\')">‹</button>' +
    ...
    '<div class="model-tree-grid">' + detailCards.map(item =>
      '<button class="model-tree-card leaf" type="button" onclick="openDirectoryDetail(\'' + escapeHtml(preview.vendor_key) + '\',\'' + escapeHtml(item.tool_key) + '\',this)">' +
