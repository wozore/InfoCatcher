 ...
 * 这是可再生的派生投影：发布失败降级（诊断经结果对象结构化字段携带，src 禁 console.*），
 * 不进事务回滚链；comparison 读端校验 + 空默认兜底，下次 catalog 落盘自动刷新。
 */
...
/** 发布到共享段；失败降级（派生投影，可再生），不抛；失败诊断经 publish_warning 结构化携带。 */
function publishCatalogReleaseDates(snapshot) {
  ...
  if (!result.ok) {
    return { ...result, publish_warning: (result.errors || [result.error]).join('; ') };
  }
  return result;
}
