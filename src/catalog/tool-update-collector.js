...
/** 极简 HTML → 文本：去除脚本/样式/标签与实体，压缩空白，保留 changelog 正文日期。 */
function htmlToText(html) {
  return String(html || '')
    ...
    .replace(/<[^>]+>/g, ' ')
    // 移除 GitHub 风格贡献图摘要 "N contributions between <date> and <date>"，其日期不是 changelog 条目。
...
async function fetchHtmlText(url, options = {}) {
  ...
  try {
    ...
    clearTimeout(timer);
    if (!response?.ok) return errorResult(`UPDATE_HTML_HTTP_${Number(response?.status || 0)}`, 'HTML 抓取失败', { status: Number(response?.status || 0) });
