...
**用户拍板**：(1) 合成证据 = 回读 approved 摘要（不调 Tavily）；(2) 不自动 apply（batch 只写预览，`concept apply` 人工确认才写 glossary）；(3) vibe-hub 自动补充证据 + 本地缓存 `data/manual/vibe-hub-cache.json`，串行 ≥500ms 节流，仅缓存不加硬上限/开关；(4) TTL 3 天 + 定时刷新在 YouTube 采集前 1h（每 3 天北京 19:00 / UTC `0 11 */3 * *`），新概念术语由 cache-miss 自动跟上。
