...
## 2026-08-09 · 热点候选轻量历史与采集批次切换

**背景**：`min-candidates.json` 原来会保留所有旧候选，重新采集时历史候选会继续参与统计和审核，导致候选文件长期累积，也曾让 CI 测试把旧候选和本批候选一起计数。

### 实际变更

- [x] 新增 [min-history.js](src/news/min/min-history.js) 与 `data/news/runtime/min-candidates-history.json`：每批历史只保存候选 `id`、`title`，批次时间按北京时间记录为 `YYYY-MM-DD-HH:MM:SS`。
- [x] 历史文件只保留最近 30 批，同一批次时间重复写入时保持幂等；历史只用于查看，不可恢复完整候选详情。
- [x] [pipeline-min.js](src/news/min/pipeline-min.js) 在采集前先保存上一批轻量摘要，再清空当前候选层；历史保存或清空失败时不启动采集，避免旧候选无记录丢失。
- [x] 增加 `minHistoryIn/minHistoryOut` 与现有候选层注入点，测试可以完全使用内存存根，不触碰正式历史文件。
- [x] 当前批次不再继承上一批候选；`mergeCandidatesMin` 的同批次状态保留语义未改。

### 验证结果

- [x] 轻量历史与管线测试 **5/5 通过**。
- [x] GitHub Actions 同款测试集 **76/76 通过**，未调用真实 DeepSeek。
- [x] 测试生成的轻量历史文件已清理，正式运行时数据未作为测试输入写回。

### 已知边界

- [~] 历史只保存 `id/title`，不能恢复上一批的描述、评分、审核状态、总结或本地化内容。
- [~] 第一次启用时，当前已有候选会被视为一个批次保存摘要；下一次采集开始使用新的批次边界。
